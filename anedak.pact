...
(module anedak GOVERNANCE
  ...
  (defun transfer-create:string
    ...
    ;   (install-capability (coin.TRANSFER "test-pool" "k:emily" 10.0))
    ;
    ; (coin.transfer "test-pool" "k:emily" 10.0)
    )
  )
  ...
  (defun transfer:string
    ...
    ; (with-capability (TRANSFER "test-pool" receiver amount)
    ;   (transfer "test-pool" "k:emily" 10.0)
    ;
    ; )
      ...
      (if (= "" r) true
        (if (= "k" r)
          ...
          (enforce false
            ...
            (defun enforce-reserved2 (accountId:string guard:guard)
    @doc "Enforce reserved account name protocols."
    (let ((r (check-reserved accountId)))
      (if (= "" r) true
        (if (= "k" r)
